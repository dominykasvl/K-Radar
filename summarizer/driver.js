...
Summarizer.Utility.calculateSimilarity = function (sentence1, sentence2) {
  ...
  var intersection = words1.filter((value) => words2.includes(value));
...
Summarizer.Utility.calculatePageRank = function (
  graph,
  maxIterations,
  dampingFactor,
  delta,
  ...
  for (var iter = 0; iter < maxIterations; ++iter) {
    maxPRChange = Summarizer.Utility.runPageRankOnce(
      graph,
      pageRankStruct,
      totalWeight,
      totalNumNodes,
...
Summarizer.Utility.runPageRankOnce = function (
  graph,
  pageRankStruct,
  totalWeight,
  totalNumNodes,
...
export function summary(text) {
  ...
  var inputToSummarize = text.trim();
  if (inputToSummarize.length == 0) {
    return "No text to be summarized...";
  } else {
    ...
    var result = Summarizer.Utility.calculatePageRank(
      graph,
      configObj.maxIter,
      configObj.dampingFactor,
      configObj.delta,
    );
